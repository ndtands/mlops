...
class Input(BaseModel):
    ...
    driver: str
    model: str
project_name = 'mlops-final'
run = wandb.init(project=project_name, job_type='serving')
artifact = run.use_artifact(f'ndtands/{project_name}/model:product', type='Model')
model_dir = artifact.download()
artifact = run.use_artifact(f'ndtands/{project_name}/other-func:product', type='Transform')
other_dir = artifact.download()
artifact = run.use_artifact(f'ndtands/{project_name}/branch-func:product', type='Transform')
branch_dir = artifact.download()
artifact = run.use_artifact(f'ndtands/{project_name}/class-func:product', type='Transform')
